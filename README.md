...
--A
--1.) Find Primary position of platers whose name start with C
SELECT Distinct p.player_id AS "Player ID", 
       p.name AS "Player Name", 
       p.primary_position AS "Primary Position"
FROM player p
JOIN stats s ON p.player_id = s.player_id
WHERE p.name LIKE 'C%'
ORDER BY p.name;

--2.)Find Players who made more than 100 hits
SELECT DISTINCT p.name AS "Player Name", 
       p.primary_position AS "Primary Position", 
       b.b_hits AS "Hits"
FROM player p
JOIN batter_stats b ON p.player_id = b.stats_id 
JOIN stats s ON b.stats_id = s.player_id 
WHERE b.b_hits > 100
  AND s.batting = 'Y'
  ORDER BY b.b_hits; 
  
--3.)Find Games where Mookie Betts was MVP
SELECT g.g_date AS "Game Date", 
       COALESCE(ps.p_MVP, rs.regular_MVP) AS "MVP", 
       a.l_name AS "Coach Last Name",
        g.game_type AS "Game Type"
FROM game g
LEFT JOIN post_season ps ON g.game_id = ps.game_id 
LEFT JOIN regular_season rs ON g.game_id = rs.game_id  
JOIN all_coaches a ON g.coach_id = a.coach_id 
WHERE (ps.p_MVP = 'Mookie Betts' OR rs.regular_MVP = 'Mookie Betts')
  AND g.game_type IN ('R', 'PS')
ORDER BY g.g_date;

--4.)Find the regular season games where MVP was from Dodgers and on July
SELECT g.g_date AS "Game Date", 
       rs.regular_MVP AS "MVP",
       a.l_name AS "Coach Last Name"
FROM regular_season rs
JOIN game g ON rs.game_id = g.game_id
JOIN all_coaches a ON g.coach_id = a.coach_id 
WHERE EXTRACT(MONTH FROM g.g_date) = 7 
  AND rs.regular_MVP IS NOT NULL 
ORDER BY g.g_date;


--5.)Find Ticket Owner whose last name starts with W
SELECT t.ticket_id AS "Ticket ID",
g.g_date AS "Game Date", 
       t.customer_last AS "Customer Last Name",
       t.t_row_number AS "Row Number"
FROM game g
JOIN ticket_sale t ON g.game_id = t.game_id
WHERE t.customer_last LIKE 'W%'
ORDER BY g.g_date, t.t_row_number;

--6 Find tickets where the row number is 10
SELECT t.ticket_id AS "Ticket ID",
       g.g_date AS "Game Date", 
       t.customer_last AS "Customer Last Name",
       t.t_row_number AS "Row Number"
FROM game g
JOIN ticket_sale t ON g.game_id = t.game_id
WHERE t.t_row_number LIKE '_10' 
ORDER BY g.g_date, t.t_row_number;

--7 Find Coached whose tenure length is more than 3 years
SELECT coach_id AS "Coach ID",
       f_name AS "First Name",
       l_name AS "Last Name",
       tenure_length AS "Tenure Length"
FROM all_coaches
WHERE EXTRACT(YEAR FROM tenure_length) > 3
ORDER BY tenure_length;

--B
--B.1 Find the Player with the highest salary
SELECT name AS "Player Name",
       primary_position AS "Primary Position",
        player_salary AS "Salary"
FROM player
WHERE player_salary = (
...
--B.2 Find RP with most SO and less than 5 Loss
SELECT p.name AS "Player Name",
       p.primary_position AS "Primary Position",
       ps.p_strikeouts AS "SO",
       ps.p_loss AS "L"
FROM player p
JOIN pitching_stats ps ON p.player_ID = ps.stats_id
WHERE p.primary_position = 'RP'
AND ps.p_strikeouts >= 50 
AND ps.p_loss < 5
AND ps.stats_id IN (
    SELECT ps2.stats_id
    FROM pitching_stats ps2
    WHERE ps2.p_strikeouts >= 50
    AND ps2.p_loss < 5
)
ORDER BY ps.p_strikeouts DESC, ps.p_wins DESC;

--B.3 Find Winning Percentage of SP
SELECT p.name AS "Player Name",
       p.primary_position AS "Primary Position",
       ps.p_wins AS "Wins",
       ps.p_loss AS "Losses",
       ps.p_strikeouts AS "Strike Outs",
       ROUND((ps.p_wins / (ps.p_wins + ps.p_loss)), 2) AS "Winning Percentage"
FROM player p
JOIN pitching_stats ps ON p.player_ID = ps.stats_id
WHERE p.primary_position = 'SP'
AND (ps.p_wins / (ps.p_wins + ps.p_loss)) >= 0.6  
ORDER BY "Winning Percentage" DESC; 

--B.4 Finding the Remaining Contract and Expiration Year of Batters
SELECT p.name AS "Player Name",
       p.primary_position AS "Primary Position",
       p.player_contract_remaining AS "Contract Remaining",
       EXTRACT(YEAR FROM (TO_DATE('2024-01-01', 'YYYY-MM-DD') + p.player_contract_remaining)) AS "Contract Expiration Year"
FROM player p
WHERE p.primary_position NOT IN ('SP', 'RP')
ORDER BY "Contract Expiration Year" DESC;


--B.5 Get the Players' height and weight in meteric standard of who are staying after 2028
SELECT p.name AS "Player Name",
       p.primary_position AS "Primary Position",
       FLOOR(EXTRACT(YEAR FROM (TO_DATE('2024-01-01', 'YYYY-MM-DD') + p.player_contract_remaining))) AS "Contract Expiration Year",
       p.height AS "Height (meters)",
       ROUND(p.weight * 0.453592, 2) AS "Weight (kg)"
FROM player p
LEFT OUTER JOIN pitching_stats ps ON p.player_ID = ps.stats_id
WHERE (TO_DATE('2024-01-01', 'YYYY-MM-DD') + p.player_contract_remaining) >= TO_DATE('2028-01-01', 'YYYY-MM-DD')
ORDER BY "Contract Expiration Year" DESC;

--6 Players who played in both regular season and postseason games 
SELECT p.name AS "Player Name",
       p.primary_position AS "Primary Position",
       COUNT(DISTINCT g.game_id) AS "Total Games Played"
FROM player p
JOIN game g ON p.player_id = g.coach_id
LEFT JOIN regular_season rs ON g.game_id = rs.game_id
LEFT JOIN post_season ps ON g.game_id = ps.game_id
WHERE (rs.game_id IS NOT NULL OR ps.game_id IS NOT NULL)
GROUP BY p.name, p.primary_position
HAVING COUNT(DISTINCT g.game_id) > 1
ORDER BY "Total Games Played" DESC;

--7 Average Salary of players by Primary position
SELECT p.primary_position AS "Primary Position",
       AVG(p.player_salary) AS "Average Salary"
FROM player p
GROUP BY p.primary_position
ORDER BY "Average Salary" DESC;

--8 Players who have contracts expiring in 2026
SELECT p.name AS "Player Name",
       p.primary_position AS "Primary Position",
       EXTRACT(YEAR FROM (TO_DATE('2024-01-01', 'YYYY-MM-DD') + p.player_contract_remaining)) AS "Contract Expiration Year"
FROM player p
WHERE EXTRACT(YEAR FROM (TO_DATE('2024-01-01', 'YYYY-MM-DD') + p.player_contract_remaining)) = 2026
ORDER BY "Contract Expiration Year" DESC;
